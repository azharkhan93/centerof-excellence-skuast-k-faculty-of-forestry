        ...
        <div className="hidden lg:flex items-center gap-12 text-sm font-medium">
            {NAV_LINKS.map((link) => (
                <div key={link.href} className="relative group">
                    {link.subLinks ? (
                        <div className="flex items-center gap-1 cursor-default hover:text-orange-400 transition-colors uppercase tracking-widest text-[10px] py-2">
                            {t(link.labelKey)}
                            ...
                            <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform scale-95 group-hover:scale-100">
                                <div className="bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-2xl p-4 min-w-[200px] shadow-2xl">
                                    <div className="flex flex-col gap-2">
                                        {link.subLinks.map((sub) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    {IconComponent && <IconComponent className="w-5 h-5 text-orange-400 group-hover/sub:scale-110 transition-transform" />}
